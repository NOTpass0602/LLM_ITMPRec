...
if __name__ == '__main__':
    ...
    if args.export_dir:
        ...
        with torch.no_grad():
            ...
            user_emb = embedding['user'].detach().cpu()
            item_emb = embedding['item'].detach().cpu()
        torch.save(user_emb, os.path.join(args.export_dir, f"user_emb-v2-{args.embed_size}.pt"))
        torch.save(item_emb, os.path.join(args.export_dir, f"item_emb-v2-{args.embed_size}.pt"))
        if args.embed_size == 64:
            torch.save(user_emb, os.path.join(args.export_dir, "user_emb-v2-64.pt"))
            torch.save(item_emb, os.path.join(args.export_dir, "item_emb-v2-64.pt"))
        logging.info('exported embeddings to {}'.format(args.export_dir))
